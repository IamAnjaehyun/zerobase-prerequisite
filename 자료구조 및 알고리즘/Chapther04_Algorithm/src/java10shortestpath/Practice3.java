...
//모든 도시의 쌍 (A,B)에 대해서 도시 A에서 B로 가는데 필요한 비용의 최솟값을 구하는 프로그램을 작성하시오. > FloydWarshall
...
// city : 5
// bus : 14
// busInfo : {{1,2,2},{1,3,3},{1,4,1},{1,5,10},{2,4,2},{3,4,1},{3,5,1},
...
public class Practice3 {
    ...
    public static void solution(int city, int bus, int[][] busInfo) {
        ...
        //간선 정보 업데이트, 같은 경로 중 최소값 뽑음
        for (int i = 0; i < bus; i++) {
            dist[busInfo[i][0]][busInfo[i][1]] = Math.min(dist[busInfo[i][0]][busInfo[i][1]], busInfo[i][2]); //원래 값이랑 새로 들어오는 값 비교해서 더 작은값 업데이트
        }
