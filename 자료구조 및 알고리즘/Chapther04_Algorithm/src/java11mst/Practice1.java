...
//n개의 행성 사이를 n-1 개의 터널로 연결하는데 드는 최소 비용을 구하는 프로그램을 작성하세요. -> MST
...
public class Practice1 {
    ...
    static class Edge implements Comparable<Edge> {
        ...
        public int compareTo(Edge o) {
            return weight - o.weight; //낮은 weight 순으로
        }
    }
    ...
    public static int solution(int[][] data) {
        ...
        Point[] points = new Point[n]; //n개의 행성에 대한 좌표 관리할 배열

        for (int i = 0; i < n; i++) {
            points[i] = new Point(i, data[i][0], data[i][1], data[i][2]); //i번째 행성에 대해 x좌표 y좌표 z좌표 넣어줌
        }

        edges = new ArrayList<>();

        //x축 기준 간선 추가
        Arrays.sort(points, (p1, p2) -> p1.x - p2.x); //x좌표 기준으로 정렬
        for (int i = 0; i < n - 1; i++) {
        ...
        //y축 기준 간선 추가
        Arrays.sort(points, (p1, p2) -> p1.y - p2.y); //x좌표 기준으로 정렬
        for (int i = 0; i < n - 1; i++) {
        ...
        //z축 기준 간선 추가
        Arrays.sort(points, (p1, p2) -> p1.z - p2.z); //x좌표 기준으로 정렬
        for (int i = 0; i < n - 1; i++) {
            int weight = Math.abs(points[i].z - points[i + 1].z);
            edges.add(new Edge(points[i].idx, points[i + 1].idx, weight));
        }

        Collections.sort(edges); //weight 값 기준 정렬
        //kruskal 호출해서 계산해주면 됨
